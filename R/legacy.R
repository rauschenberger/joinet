...
#' #         X1 <- X[foldid==k,,drop=FALSE]
#' #         
#' #         Y0 <- colasso_moderate(y=y0,ym=ym0,pi=pi) # trial
#' #         for(j in seq_along(pi)){
#' #             glmnet <- glmnet::glmnet(y=Y0[,j],x=X0,alpha=alpha)
...
#' #' NA
#' #'
#' colasso_compare <- function(y,Y,X,plot=TRUE,nfolds.int=10){
#'   
...
#'     cat("i =",i,"\n")
#'     fit <- colasso(y=y[fold!=i],Y=Y[fold!=i,],X=X[fold!=i,],alpha=1,nfolds=nfolds.int)
...
#' 
#' 
#' # colasso_compare <- function(y,ym,X,plot=TRUE,nfolds.int=10){
#' #   
#' #   fold <- sample(x=rep(x=seq_len(5),length.out=length(y)))
#' #   pred <- matrix(data=NA,nrow=length(y),ncol=8)
#' #   select <- list()
#' #   for(i in sort(unique(fold))){
#' #     cat("i =",i,"\n")
#' #     fit <- colasso(y=y[fold!=i],ym=ym[fold!=i],X=X[fold!=i,],alpha=1,nfolds=nfolds.int)
#' #     for(j in seq_along(fit)){
#' #       pred[fold==i,j] <- glmnet::predict.glmnet(object=fit[[j]],
#' #                                                 newx=X[fold==i,],
#' #                                                 s=fit[[j]]$lambda.min,
