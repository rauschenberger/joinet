...
predict.bilasso <- function(x,newx,type="response",...){
  ...
  gaussian <- stats::pnorm(q=pred_y,mean=x$cutoff,sd=x$sd.y) # new
  ...
  frame <- data.frame(gaussian=gaussian,binomial=binomial,pi=pi,base=base)
  return(frame)
}
...
bilasso_compare <- function(y,cutoff,X,type.measure="deviance",res=100){
  ...
  cols <- c("gaussian","binomial","pi","base")
